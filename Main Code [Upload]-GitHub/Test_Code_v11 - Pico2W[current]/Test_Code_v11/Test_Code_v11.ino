/*
  ================================================================
  SPOTIFY HUB75 CLOCK/DISPLAY - STAGE 8 / v11 (Pico 2 W smoothness pass)
  ...
  WHAT CHANGED FOR v11 (moved to Pico 2 W)
    Only the disc animation was touched this version, on request -
    the clock/weather screen redesign is planned for the next one.
    1. drawRecordFast() used to scan the full 64x64 grid THREE
       separate times per frame: once for the main disc using the
       precomputed integer discPixels[] radius, then AGAIN with
       fresh float distance math just to redraw the center label
       brighter, then AGAIN with fresh float distance math just to
       punch out the spindle hole. Two of those three passes were
       pure repeated work - discPixels already has every pixel's
       radius precomputed, so all four zones (rim / label / spindle
       / normal artwork) are now decided in ONE pass. See the
       comment block above drawRecordFast() for details.
    2. The rim highlight ring's 180 cosf/sinf calls per frame are
       gone - that ring is a fixed set of screen coordinates that
       never changes with rotation, so it's computed once at
       startup into rimHighlightX[]/Y[] and just looked up after
       that (prepareRimHighlightLUT()).
    3. DISPLAY_FPS raised from 40 to 80. The Pico W (RP2040,
       Cortex-M0+) has no hardware floating-point unit, so every
       cosf/sinf/float-multiply in this file was being software-
       emulated - genuinely slow. The Pico 2 W (RP2350, Cortex-M33)
       has a real FPU, so the same float-heavy code runs drastically
       faster before even counting the pass-merging above. If 80
       ever looks unstable on your panel, drop it back down - there's
       nothing else tying this number to anything.
    4. Everything else (WiFi/offline handling, clock/weather scene,
       artwork pipeline, transitions, Spotify polling, TLS buffer
       fix) is unchanged from the previous version.

  IMPORTANT
    Tools -> Board: Raspberry Pi Pico 2 W
    Tools -> CPU Speed: whatever default/max your core package offers
    Tools -> Flash Size: pick one with filesystem space (e.g. 4MB
    Sketch / rest FS) - LittleFS is still used for the saved
...
// core0 -> core1: true when WiFi.status() reports connected (link to
// the router is fine) but the last Spotify poll got no real response
// (status <= 0 - timeout/DNS/connection refused). This is the "WiFi
// is up but there's no actual internet" case, which wifiReconnecting
// alone can't detect since the router link itself never drops.
...
//
// v11 SMOOTHNESS PASS (Pico 2 W): the old version scanned the full
// 64x64 grid THREE separate times per frame - once for the main disc
// (using the precomputed integer discPixels[] radius), then AGAIN
// with fresh float distance math to redraw the center label brighter,
// then AGAIN with fresh float distance math to punch out the spindle
// hole. Two of those three passes were pure repeated work - discPixels
// already has the radius for every pixel, so all three zones (rim /
// label / spindle / normal artwork) are now decided in one single
// pass. The rim highlight ring's 180 cosf/sinf calls per frame are
// also gone - that ring is a fixed set of screen coordinates that
// never changes with rotation, so it's computed once at startup into
// rimHighlightX/Y and just looked up every frame after that.
// ================================================================
...
// Fixed radius thresholds, in the same 1/256-pixel units as
// discPixels[].radius256 - these never change, only which zone a
// given pixel falls into depends on its (already precomputed) radius.
...
// Fixed ring of screen coordinates for the rim highlight overlay -
// same geometry the old per-frame "for (deg=0;deg<360;deg+=2)" loop
// produced, just computed once instead of every frame.
...
void drawRecordFast(uint16_t* image, float rotationDegrees, uint8_t brightness, uint16_t rimColor) {
  ...
  for (int i = 0; i < WIDTH * HEIGHT; i++) {
    ...
    if (r256 <= DISC_SPINDLE_RADIUS256) {
      matrix.drawPixel(x, y, 0); // spindle hole
      continue;
    }
    ...
    if (r256 <= DISC_LABEL_RADIUS256) {
      // Center label area - full brightness, no vignette (matches the
      // old second-pass overwrite exactly, just done in the same pass)
...
void loop() {
  ...
  if (now - lastSpotifyPoll >= SPOTIFY_POLL_MS) {
    ...
    // Captured before this request: whether either connectivity signal
    // was bad as of the last known state. If this poll succeeds and the
    // same track is still playing, this is what triggers a forced
    // artwork re-fetch below - otherwise a same-track poll after
    // reconnecting never sets artworkDownloadPending (URL didn't
    // change), leaving the display stuck off SCENE_DISC until the
    ...
    if (https.begin(spotifyPollClient, "https://api.spotify.com/v1/me/player/currently-playing")) {
      ...
      // status <= 0 means the request itself failed (timeout/DNS/
      // connection refused) rather than the server replying with an
      // error - that's "WiFi link is up but no real internet". Any
      // positive status, even an error one, means we did reach a
      // server, so the internet is fine.
      ...
      if (status == 401) {
        ...
        if (item >= 0) {
          ...
          if (currentArtworkURL.length() && currentArtworkURL != oldArtwork) {
            ...
            // Same track as before an outage - the URL match above never
            // fires, so without this, core1 has to keep trusting its
            // pre-outage artwork buffer to get back to SCENE_DISC. Forcing
            // a redownload here instead re-runs the exact same tested
            // path a track change already uses (download -> decode ->
            // stagingReady -> core1 transition), which is what actually
            // brings the disc back reliably.
            artworkDownloadPending = true;
      ...
      internetDown = true; // couldn't even open a connection
    }
...
// What to show when nothing is confirmed playing right now - this is
// also the fallback used during a WiFi outage (see below), since a
// dropped connection means currentlyPlaying can't be trusted either.
...
void loop1() {
  ...
  if (!transitionActive) {
    if (wifiReconnecting != previousWifiReconnecting) {
      ...
      // does not trigger it here - the final else-if below (which runs
      // on the following frames while wifiReconnecting stays true) is
      // what actually pulls the display off a stale SCENE_DISC.
      ...
      // While offline (WiFi fully down, OR WiFi up but no real internet -
      // Spotify polls getting status <= 0), never sit on SCENE_DISC -
      // currentlyPlaying can be stale since polling is failing either
      // way, so treat "no connection" the same as "not playing".
