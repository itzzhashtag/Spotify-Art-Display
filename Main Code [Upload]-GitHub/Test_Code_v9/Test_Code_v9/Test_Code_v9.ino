/*
  ================================================================
  SPOTIFY HUB75 CLOCK/DISPLAY - STAGE 6 (no watchdog)
  ...
  WHAT CHANGED FROM STAGE 6 (watchdog version)
    1. Removed rp2040.wdt_begin()/wdt_reset() entirely, on your
       call - you only wanted the dual-core split for display
       smoothness, not a hardware reset safety net. If core0 or
       core1 genuinely hangs now, the chip will NOT auto-reboot;
       it'll just sit there. Decide later if you want that back.
    2. Logs rp2040.resetReason() once at boot so if the board DOES
       reset on its own (brownout, crash, etc - nothing to do with
       the watchdog since that's gone now), you can see why in
       Serial Monitor on the next boot.
    3. TLS/heap fix for the polling path: WiFiClientSecure was
       being constructed fresh every 5s poll (full TLS handshake +
       large default RX/TX buffers ~16KB each). That repeated
       alloc/free on a small, fragmented heap is a very plausible
       cause of a hard crash-reset that looks like "random freeze
       then reboot" even with no watchdog involved. Fixed by:
         - reusing ONE persistent WiFiClientSecure for polling
           instead of a new one every 5s
         - calling setBufferSizes() to shrink TLS buffers from the
           ~16KB default down to something sane for small JSON/
           JPEG payloads
       Applied the same buffer shrink to the token-refresh and
       artwork-download clients too, since those also alloc a
       fresh WiFiClientSecure each time they're called.
    4. Everything else (WiFi reconnect monitor, artwork RAM
       pipeline, transitions, disc rendering) is unchanged from
       your Stage 6.
...
const uint16_t TRANSITION_MS = 900;
...
volatile bool wifiReconnecting = false; // core0 -> core1: show the ripple animation
...
// later drop - core1 shows the ripple animation the whole time
// wifiReconnecting is true, at boot or mid-session alike.
// ================================================================
...
// ================================================================
// WEATHER FETCH (core0 only) - plain HTTP, no TLS needed.
// Writes only primitive values into the shared* volatiles above -
// never touches core1's matrix and never hands core1 a String.
// ================================================================
...
bool fetchWeatherIntoShared() {
  ...
  HTTPClient http;
  String url = "http://api.open-meteo.com/v1/forecast?latitude=" + String(WEATHER_LAT, 4) +
...
// ================================================================
// WIFI RIPPLE ANIMATION (core1 only)
// Continuously repeating - 3 rings offset in phase so all 3 are
// visibly at different sizes at any given moment, instead of
// growing once and freezing stacked on top of each other.
// ================================================================
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("PICO W SPOTIFY HUB75 - STAGE 6 (no WDT)");
  Serial.println("========================================");
...
const char* weatherLabelForCode(int code) {
  ...
  if (code == 1 || code == 2) return "P.Cldy";
...
// Small hollow "degree" mark drawn by hand since the default 5x7 font
// has no glyph for it - keeps "44 Cloudy" reading as "44deg Cloudy".
...
void drawClockScene(uint8_t sceneBrightness) {
  ...
  float breathe = 0.55f + 0.45f * sinf(millis() * 0.0022f); // slow pulse, never fully off
  ...
  if (sharedWeatherValid) {
    ...
    label = weatherLabelForCode(sharedWeatherCode);
  } else {
  ...
  char lineStr[20];
  snprintf(lineStr, sizeof(lineStr), "%s %s", tempStr, label);
  int lineWidth = strlen(lineStr) * 6;
  if (lineWidth > WIDTH - 2) lineWidth = WIDTH - 2; // stay inside the 64px bound
  ...
  matrix.setCursor(lineX, lineY);
  matrix.print(lineStr);

  if (sharedWeatherValid) {
    int degX = lineX + strlen(tempStr) * 6;
    int degY = lineY - 5;
...
SceneType currentScene = SCENE_IDLE;
...
void loop1() {
  ...
  if (!transitionActive) {
    if (wifiReconnecting != previousWifiReconnecting) {
      // WiFi status just changed - fade to/from the ripple animation
      if (wifiReconnecting) {
  ...
  renderScene(currentScene, currentArtworkPtr, currentDominantColor, discRotation, ARTWORK_MAX_BRIGHTNESS);
}
