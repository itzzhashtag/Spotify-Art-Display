...
void drawSpotifyIdle(uint8_t brightness) {
  ...
  uint16_t green = scale565(rgb565(0, 60, 0), brightness);
...
void loop1() {
  ...
  if (!transitionActive) {
    if (wifiReconnecting != previousWifiReconnecting) {
      ...
      // While WiFi is down (post-first-connect drop), never sit on
      // SCENE_DISC - currentlyPlaying can be stale since polling is
      // failing, so treat "no connection" the same as "not playing".
      SceneType wantScene = (wifiReconnecting && hasConnectedOnce)
        ? computeIdleOrClockScene()
        : computeFallbackScene();
